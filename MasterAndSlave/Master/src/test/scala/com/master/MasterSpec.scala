...
//      val result = Await.result(futureResult, 10 seconds).asInstanceOf[List[Map[String, (String, Double)]]]
//
...
//  }
  ...
  describe("RunMapReduceRequest Test") {
    it("should receive res based on the Seq group [A]"){
      ...
      val result_1 = Await.result(futureResult_1, 10 seconds).asInstanceOf[List[Map[String, Vector[(String, Double)]]]]
      val ans = Map("A" -> Vector(("A",3.0), ("A",6.0), ("A",9.0), ("A",12.0), ("A",15.0), ("A",18.0)),
        "C" -> Vector(("C",2.0), ("C",5.0), ("C",8.0), ("C",11.0), ("C",14.0), ("C",17.0), ("C",20.0)),
        "B" -> Vector(("B",1.0), ("B",4.0), ("B",7.0), ("B",10.0), ("B",13.0), ("B",16.0), ("B",19.0)))

      val seqA = result_1.map(_("A")).flatten.map(_._2)
      val futureResult_2 = client.mapReduce("A", seqA)
